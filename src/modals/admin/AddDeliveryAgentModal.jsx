import React, { useContext, useState, useEffect } from "react";
import { AppContext } from "../../ContextAPI";
import { useDispatch } from "react-redux";
import { addDeliveryAgentAction } from "../../Api/admin/admin.api";

const AddDeliveryAgentModal = () => {
  const { AdminPage, setAdminPage, modalConfig, setModalConfig,setCurrentOrder,currentOrder } =
    useContext(AppContext);
  const dispatch = useDispatch();
  const [formdata, setFormdata] = useState({
    name: "",
    contact_detail:""
  });

  const handleChange = (e) => {
    setFormdata({ ...formdata, [e.target.name]: e.target.value });
  };

  useEffect(() => {
    // setFormdata({ ...formdata, assetName: "USDT", networkName: "Solana" });
  }, []);

  return (
    <div className="fixed grid h-[100%] z-20 bg-modal-bg place-items-center w-full backdrop-blur-sm lg:px-0 px-4 top-0 font-spacegrotesk">
      <div className="bg-white shadow-2xl min-h-[200px] rounded-md w-2/6 p-4">
        <p className="font-spacegrotesk my-3 ">Add delivery agent || {currentOrder?.mega_id}</p>

        <div className="w-full flex border border-gray-400 p-2 my-2 rounded-md">
          <input
            type="text"
            name="name"
            id=""
            className="outline-none  w-full text-[12px]"
            placeholder="Delivery agent name"
            onChange={(e) => handleChange(e)}
          />
        </div>

        <div className="w-full flex border border-gray-400 p-2 my-2 rounded-md">
          <input
            type="number"
            name="contact_detail"
            id=""
            className="outline-none  w-full text-[12px]"
            placeholder="Enter delivery agent phone number"
            onChange={(e) => handleChange(e)}
          />
        </div>

        <div className="flex justify-end     space-x-2 my-3">
          <button
            className="font-african text-black border border-green-500  text-[10px] bg-white p-2 rounded-sm capitalize"
            onClick={() => setModalConfig(null)}
          >
            Cancel
          </button>
          <button
            className="font-african text-white text-[10px] bg-green-500 p-2 rounded-sm capitalize"
            onClick={() => dispatch(addDeliveryAgentAction({id:currentOrder?.order_id, formdata:formdata,setModalConfig:setModalConfig }))}
          >
            Add
          </button>
        </div>
      </div>
    </div>
  );
};

export default AddDeliveryAgentModal;
